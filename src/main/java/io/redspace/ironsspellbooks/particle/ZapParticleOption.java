...
public class ZapParticleOption implements ParticleOptions {
   public static final Codec<ZapParticleOption> CODEC = Codec.INT_STREAM
      .comapFlatMap(
         p_121967_ -> Util.m_137539_(p_121967_, 3).map(vec3 -> new ZapParticleOption(new Vec3(vec3[0] / 10.0F, vec3[1] / 10.0F, vec3[2] / 10.0F))),
         p_121924_ -> IntStream.of(
            (int)(p_121924_.destination.f_82479_ * 10.0), (int)(p_121924_.destination.f_82480_ * 10.0), (int)(p_121924_.destination.f_82481_ * 10.0)
         )
      );
   public static final Deserializer<ZapParticleOption> DESERIALIZER = new Deserializer<ZapParticleOption>() {
   ...
   public ParticleType<ZapParticleOption> m_6012_() {
      return (ParticleType<ZapParticleOption>)ParticleRegistry.ZAP_PARTICLE.get();
   }
   ...
   private static Vec3 fromNetwork(FriendlyByteBuf buf) {
      return new Vec3(buf.readInt() / 10.0F, buf.readInt() / 10.0F, buf.readInt() / 10.0F);
   }

   private static void toNetwork(Vec3 vec, FriendlyByteBuf buf) {
      buf.writeInt((int)(vec.f_82479_ * 10.0));
      buf.writeInt((int)(vec.f_82480_ * 10.0));
      buf.writeInt((int)(vec.f_82481_ * 10.0));
   }
}
