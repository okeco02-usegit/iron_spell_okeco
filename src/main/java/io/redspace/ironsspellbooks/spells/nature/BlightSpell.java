...
public class BlightSpell extends AbstractSpell {
   private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath("irons_spellbooks", "blight");
   ...
   public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
      return List.of(
         Component.m_237110_(
            "ui.irons_spellbooks.reduced_healing", new Object[]{Utils.stringTruncation((1 + this.getAmplifier(spellLevel, caster)) * -0.1F * -100.0F, 1)}
         ),
         Component.m_237110_(
            "ui.irons_spellbooks.reduced_damage", new Object[]{Utils.stringTruncation((1 + this.getAmplifier(spellLevel, caster)) * -0.05F * -100.0F, 1)}
         ),
   ...
   public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
      if (playerMagicData.getAdditionalCastData() instanceof TargetEntityCastData targetData) {
         ...
         if (targetEntity != null) {
            targetEntity.m_7292_(
               new MobEffectInstance((MobEffect)MobEffectRegistry.BLIGHT.get(), this.getDuration(spellLevel, entity), this.getAmplifier(spellLevel, entity))
            );
         }
      }
