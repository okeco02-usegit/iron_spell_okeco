...
public class SlowSpell extends AbstractSpell {
   private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath("irons_spellbooks", "slow");
   ...
   public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
      return List.of(
         Component.m_237110_("ui.irons_spellbooks.slowed", new Object[]{Utils.stringTruncation((1 + this.getAmplifier(spellLevel, caster)) * 0.1F * 100.0F, 1)}),
   ...
   public boolean checkPreCastConditions(Level level, int spellLevel, LivingEntity entity, MagicData playerMagicData) {
      if (Utils.preCastTargetHelper(level, entity, playerMagicData, this, 32, 0.35F)) {
         ...
         TargetedAreaEntity area = TargetedAreaEntity.createTargetAreaEntity(
            level, target.m_20182_(), radius, ((MobEffect)MobEffectRegistry.SLOWED.get()).m_19484_(), target
         );
            ...
            targetEntity.f_19853_
               ...
               .forEach(
                  victim -> {
                     if (targets.get() < 5
                        && victim != entity
                        && victim.m_20280_(targetEntity) < radius * radius
                        && !DamageSources.isFriendlyFireBetween(entity, victim)) {
                        victim.m_7292_(
                           new MobEffectInstance(
                              (MobEffect)MobEffectRegistry.SLOWED.get(), this.getDuration(spellLevel, entity), this.getAmplifier(spellLevel, entity)
                           )
                        );
   ...
   public Vector3f getTargetingColor() {
      return Utils.deconstructRGB(((MobEffect)MobEffectRegistry.SLOWED.get()).m_19484_());
   }
}
