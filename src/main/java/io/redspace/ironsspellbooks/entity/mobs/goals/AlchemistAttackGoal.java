...
public class AlchemistAttackGoal extends WizardAttackGoal {
   ...
   public static final List<MobEffect> ATTACK_POTIONS = List.of(
      MobEffects.f_19613_, MobEffects.f_19610_, MobEffects.f_19620_, MobEffects.f_19597_, MobEffects.f_19599_
   );

   public AlchemistAttackGoal(
      IMagicEntity abstractSpellCastingMob, double pSpeedModifier, int minAttackInterval, int maxAttackInterval, float throwRange, float potionBias
   ) {
   ...
   public AlchemistAttackGoal setSpells(
   ...
   protected void doSpellAction() {
      if (this.mob.m_20280_(this.target) < this.throwRangeSqr && this.mob.m_217043_().m_188501_() < this.potionBias) {
         ...
         ItemStack potion = new ItemStack(Items.f_42736_);
         ...
         if (this.hasLineOfSight && this.mob.m_217043_().m_188501_() * (attackWeight + supportWeight) > supportWeight) {
            int amplifier = (this.mob.m_217043_().m_188501_() < 0.75F ? 0 : 1)
               + (this.target.m_21233_() > 30.0F ? (this.mob.m_217043_().m_188501_() < 0.5F ? 0 : 1) : 0);
            MobEffect effect = this.target.m_21222_() ? MobEffects.f_19601_ : MobEffects.f_19602_;
            if (this.mob.m_217043_().m_188501_() < 0.45F) {
               for (int i = 0; i < ATTACK_POTIONS.size(); i++) {
                  ...
                  if (!this.target.m_21023_(ATTACK_POTIONS.get(p))) {
            ...
            PotionUtils.m_43552_(potion, List.of(new MobEffectInstance(effect, effect.m_8093_() ? 0 : 200, amplifier)));
            PotionUtils.m_43549_(potion, Potions.f_43599_);
         } else {
         ...
         thrownpotion.m_6686_(d0, d1 + d3 * 0.2, d2, (float)Mth.m_14085_(0.5, 1.25, this.mob.m_20280_(targetedEntity) / this.throwRangeSqr), 8.0F);
         if (!this.mob.m_20067_()) {
            this.mob
               ...
               .m_6263_(
                  ...
                  this.mob.m_5720_(),
                  1.0F,
                  0.8F + this.mob.m_217043_().m_188501_() * 0.4F
               );
         }
