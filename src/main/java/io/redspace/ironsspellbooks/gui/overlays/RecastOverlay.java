...
public class RecastOverlay implements IGuiOverlay {
   ...
   public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
      if (!Minecraft.m_91087_().f_91066_.f_92062_ && !Minecraft.m_91087_().f_91074_.m_5833_()) {
         if (ClientMagicData.getRecasts().hasRecastsActive()) {
            ...
            RecastOverlay.Anchor anchor = (RecastOverlay.Anchor)ClientConfigs.RECAST_ANCHOR.get();
            ...
            for (int castIndex = 0; castIndex < activeRecasts.size(); castIndex++) {
               ...
               poseStack.m_85836_();
               poseStack.m_252880_(barX - 18, barY - 2, 0.0F);
               poseStack.m_85841_(0.85F, 0.85F, 0.85F);
               RenderSystem.setShaderTexture(0, spell.getSpellIconResource());
               guiGraphics.m_280163_(spell.getSpellIconResource(), 0, 0, 0.0F, 0.0F, 16, 16, 16, 16);
               RenderSystem.setShaderTexture(0, TEXTURE);
               guiGraphics.m_280163_(TEXTURE, -2, -2, 116.0F, 0.0F, 20, 20, 256, 256);
               poseStack.m_85849_();
               ...
               for (int i = 0; i < total; i++) {
                  ...
                  if (i + 1 < total) {
                     guiGraphics.m_280163_(TEXTURE, connectorX, barY + 3, 109.0F, 8.0F, 6, 4, 256, 256);
                  }

                  boolean charged = i < remaining;
                  guiGraphics.m_280163_(TEXTURE, orbX, barY, 99 + (charged ? 0 : 10), 26.0F, 10, 10, 256, 256);
                  if (charged) {
                  ...
                  guiGraphics.m_280163_(TEXTURE, orbX, barY, 99.0F, 5.0F, 10, 10, 256, 256);
               }

               int textX = barX + 16 * total;
               guiGraphics.m_280488_(
                  Minecraft.m_91087_().f_91062_,
                  formatTime(recastInstance.getTicksRemaining(), recastInstance.getTicksToLive()),
                  textX,
                  barY + (10 - 9) / 2,
                  ChatFormatting.WHITE.m_126665_()
               );
            }
   ...
   public static void countBossBars(BossEventProgress event) {
      instance.bossbarOffset = instance.bossbarOffset + event.getIncrement();
   }
