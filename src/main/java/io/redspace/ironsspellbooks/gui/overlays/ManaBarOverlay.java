...
public class ManaBarOverlay implements IGuiOverlay {
   ...
   public void render(ForgeGui gui, GuiGraphics guiHelper, float partialTick, int screenWidth, int screenHeight) {
      if (!Minecraft.m_91087_().f_91066_.f_92062_ && !Minecraft.m_91087_().f_91074_.m_5833_()) {
         ...
         if (shouldShowManaBar(player)) {
            ...
            guiHelper.m_280163_(TEXTURE, barX, barY, spriteX, spriteY, imageWidth, 21, 256, 256);
            guiHelper.m_280218_(TEXTURE, barX, barY, spriteX, spriteY + 21, (int)(imageWidth * Math.min((double)mana / maxMana, 1.0)), 21);
            String manaFraction = mana + "/" + maxMana;
            int textX = (Integer)ClientConfigs.MANA_TEXT_X_OFFSET.get() + barX + imageWidth / 2 - (int)(((mana + "").length() + 0.5) * 6.0);
            ...
            if ((Boolean)ClientConfigs.MANA_BAR_TEXT_VISIBLE.get()) {
               guiHelper.m_280488_(Minecraft.m_91087_().f_91062_, manaFraction, textX, textY, TEXT_COLOR);
            }
         }
      }
   }
   ...
   public static boolean shouldShowManaBar(Player player) {
      ...
      return !player.m_5833_()
         ...
         && (anchor != ManaBarOverlay.Anchor.XP || ((LocalPlayer)player).m_108634_() == 0.0F)
         && (
            display == ManaBarOverlay.Display.Always
               ...
               || ClientMagicData.getPlayerMana() < player.m_21133_((Attribute)AttributeRegistry.MAX_MANA.get())
         );
   }
