...
public class PlayerCooldowns {
   ...
   public void tick(int actualTicks) {
      List<Entry<String, CooldownInstance>> spells = this.spellCooldowns
         .entrySet()
         .stream()
         .filter(x -> this.decrementCooldown(x.getValue(), actualTicks))
         .toList();
   ...
   public ListTag saveNBTData() {
      ...
      this.spellCooldowns.forEach((spellId, cooldown) -> {
         if (cooldown.getCooldownRemaining() > 0) {
            ...
            ct.m_128359_("id", spellId);
            ct.m_128405_("scd", cooldown.getSpellCooldown());
            ct.m_128405_("cdr", cooldown.getCooldownRemaining());
            listTag.add(ct);
         }
      });
      return listTag;
   ...
   public void loadNBTData(ListTag listTag) {
      if (listTag != null) {
         listTag.forEach(tag -> {
            ...
            String spellId = t.m_128461_("id");
            int spellCooldown = t.m_128451_("scd");
            int cooldownRemaining = t.m_128451_("cdr");
