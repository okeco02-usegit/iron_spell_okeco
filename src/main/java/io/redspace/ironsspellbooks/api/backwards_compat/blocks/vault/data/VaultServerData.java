...
public class VaultServerData {
   ...
   public static final Codec<Set<UUID>> UUID_SET = Codec.list(UUIDUtil.f_235867_).xmap(Sets::newHashSet, Lists::newArrayList);
   static Codec<VaultServerData> CODEC = RecordCodecBuilder.create(
      p_338073_ -> p_338073_.group(
            ...
            Codec.LONG.optionalFieldOf("state_updating_resumes_at", 0L).forGetter(p_323634_ -> p_323634_.stateUpdatingResumesAt),
            ItemStack.f_41582_.listOf().optionalFieldOf("items_to_eject", List.of()).forGetter(p_323976_ -> p_323976_.itemsToEject),
            Codec.INT.optionalFieldOf("total_ejections_needed", 0).forGetter(p_323753_ -> p_323753_.totalEjectionsNeeded)
         )
   ...
   ItemStack getNextItemToEject() {
      return this.itemsToEject.isEmpty()
         ? ItemStack.f_41583_
         : Objects.requireNonNullElse(this.itemsToEject.get(this.itemsToEject.size() - 1), ItemStack.f_41583_);
   }
   ...
   public float ejectionProgress() {
      return this.totalEjectionsNeeded == 1 ? 1.0F : 1.0F - Mth.m_184655_(this.getItemsToEject().size(), 1.0F, this.totalEjectionsNeeded);
   }
}
