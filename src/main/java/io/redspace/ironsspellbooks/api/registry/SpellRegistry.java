...
public class SpellRegistry {
   public static final ResourceKey<Registry<AbstractSpell>> SPELL_REGISTRY_KEY = ResourceKey.m_135788_(
   ...
   public static List<AbstractSpell> getEnabledSpells() {
      return REGISTRY.get().getValues().stream().filter(AbstractSpell::isEnabled).toList();
   }

   public static List<AbstractSpell> getSpellsForSchool(SchoolType schoolType) {
      return SCHOOLS_TO_SPELLS.computeIfAbsent(
         schoolType, school -> REGISTRY.get().getValues().stream().filter(spell -> spell.getSchoolType() == school).collect(Collectors.toList())
      );
   }

   public static AbstractSpell getSpell(ResourceLocation resourceLocation) {
      AbstractSpell spell = (AbstractSpell)REGISTRY.get().getValue(resourceLocation);
      return spell == null ? noneSpell : spell;
   }
