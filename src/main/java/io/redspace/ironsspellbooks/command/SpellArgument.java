...
public class SpellArgument implements ArgumentType<String> {
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
      List<String> registeredSpells = SpellRegistry.REGISTRY
         .get()
         .getEntries()
         .stream()
         .map(
            entry -> ((ResourceKey)entry.getKey()).m_135782_().m_135827_().equals("irons_spellbooks")
               ? ((ResourceKey)entry.getKey()).m_135782_().m_135815_()
         ...
         .toList();
      return SharedSuggestionProvider.m_82970_(registeredSpells, builder);
   }
